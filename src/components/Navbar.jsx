...
export default function Navbar() {
  ...
  return (
    <nav style={{
      ...
      boxShadow: scrolled ? '0 2px 20px rgba(0,0,0,0.08)' : '0 1px 8px rgba(0,0,0,0.04)',
      transition: 'box-shadow 0.3s ease',
    }}>
      <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none' }}>
        <div style={{ width: 32, height: 32, background: 'var(--navy)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'background 0.2s' }}
      ...
      <Link to="/pricing">
        <button
          className="btn-press"
          style={{ background: 'var(--navy)', color: 'var(--gold3)', border: 'none', padding: '7px 16px', fontSize: 11, fontFamily: 'Source Sans 3, sans-serif', fontWeight: 600, cursor: 'pointer', letterSpacing: '.5px', transition: 'background 0.2s' }}
          onMouseEnter={e => e.currentTarget.style.background = 'var(--navy2)'}
          onMouseLeave={e => e.currentTarget.style.background = 'var(--navy)'}
        >
          Upgrade to Pro
